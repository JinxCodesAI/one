...
import type { ServiceConfig, ProviderConfig, ModelMapping } from '../types.ts';
...
export function loadConfig(): ServiceConfig {
  ...
  if (googleApiKey) {
    providers.push({
      name: 'google',
      apiKey: googleApiKey,
      baseUrl: Deno.env.get('GOOGLE_BASE_URL') || 'https://generativelanguage.googleapis.com/v1beta',
  ...
  if (openrouterApiKey) {
    providers.push({
      ...
      defaultModel: Deno.env.get('OPENROUTER_DEFAULT_MODEL') || 'anthropic/claude-3.5-sonnet',
    });
  }
  ...
  let defaultModel = Deno.env.get('AI_DEFAULT_MODEL') || DEFAULT_CONFIG.defaultModel;
  ...
  if (!availableModels.find(m => m.name === defaultModel)) {
    ...
    console.warn(`Default model '${Deno.env.get('AI_DEFAULT_MODEL') || DEFAULT_CONFIG.defaultModel}' not available. Using '${defaultModel}' instead.`);
  }
...
export function validateConfig(config: ServiceConfig): void {
  ...
  if (!config.models.find(m => m.name === config.defaultModel)) {
    throw new Error(`Default model '${config.defaultModel}' not found in available models`);
  }

  for (const provider of config.providers) {
    if (!provider.name || !provider.apiKey) {
      throw new Error(`Provider ${provider.name || 'unknown'} missing required configuration`);
    }
  }

  for (const model of config.models) {
    if (!model.name || !model.provider || !model.modelId) {
      throw new Error(`Model ${model.name || 'unknown'} missing required configuration`);
    }
