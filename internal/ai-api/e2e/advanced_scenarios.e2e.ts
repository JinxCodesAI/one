  ...
  it("should handle dynamic scenario switching", async () => {
    ...
    try {
      ...
      const errorScenario: MockScenario = {
        ...
        generateResponse: (context, metadata) => {
          return RequestAnalyzer.generateErrorResponse(context, metadata, 'rate_limit');
        }
      };
...
describe("E2E Advanced: Request Logging", () => {
  ...
  before(() => {
    const scenario: MockScenario = {
      name: "All providers successful",
      isRequestExpected: (_context, metadata) => {
        return metadata.isApiCall && ['openai', 'google'].includes(metadata.provider);
      },
      generateResponse: (context, metadata) => {
  ...
  it("should verify request logging", async () => {
    ...
    try {
      ...
      const localRequests = requestLog.filter(req => req.context.url.includes('localhost'));
      const externalRequests = requestLog.filter(req => !req.context.url.includes('localhost'));
