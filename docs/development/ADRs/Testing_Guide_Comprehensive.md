...
This guide provides detailed instructions for creating and running tests in the One monorepo, leveraging our shared testing infrastructure and established patterns.
...
1. **Request Validation** (`isRequestExpected`): Determines if request should be mocked
2. **Response Generation** (`generateResponse`): Creates appropriate mock response
...
describe("E2E: OpenAI Provider", () => {
  ...
  before(async () => {
    ...
    const scenario = createSingleProviderSuccessScenario('openai', EXPECTED_RESPONSE);
    mockManager = new FetchMockManager(scenario);
    mockManager.start();
    
    // Setup test environment
    testEnv = await createServerTestEnvironment(startServer, createSimpleClient);
  });
...
describe("UI E2E: Chat Application Flow", () => {
  const OPENAI_RESPONSE = "Hello! This is GPT-4.1-nano responding through the UI.";
  ...
  before(async () => {
    ...
    testEnvironment = await createUITestEnvironment(startServer, scenario, config);
  });
  ...
  it("should complete full chat flow", async () => {
    ...
    await UITestHelpers.typeText(page, '[data-testid="message-input"]', 'Hello!');
...
- **Realistic responses**: Use helper utilities for realistic response structures
...
- **Specific errors**: Test specific error conditions (rate limits, invalid requests)
...
describe("E2E: OpenAI Provider", () => {
  const scenario = createSingleProviderSuccessScenario('openai', OPENAI_RESPONSE);
  // ... test implementation
});

describe("E2E: Google Provider", () => {
  const scenario = createSingleProviderSuccessScenario('google', GOOGLE_RESPONSE);
  // ... test implementation
});

describe("E2E: OpenRouter Provider", () => {
  const scenario = createSingleProviderSuccessScenario('openrouter', ANTHROPIC_RESPONSE);
  // ... test implementation
});
```
...
This testing infrastructure provides a comprehensive, maintainable, and scalable approach to testing in the One monorepo. By following these patterns and using the shared infrastructure, you can create reliable tests that provide true end-to-end coverage while remaining fast and maintainable.
