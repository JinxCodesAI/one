  ...
  describe("Network Delay Simulation", () => {
    it("should handle slow responses", async () => {
      ...
      try {
        ...
        console.log(`✅ Slow response simulation verified! Duration: ${duration}ms`);
  ...
  describe("Request Logging and Debugging", () => {
    it("should log and analyze requests", async () => {
      ...
      try {
        ...
        await client.generateText([{ role: "user", content: "Test 1" }], "gpt-4.1-nano");
        await client.generateText([{ role: "user", content: "Test 2" }], "gemini-2.5-flash");
        ...
        const openaiRequests = externalRequests.filter(req => req.metadata.provider === 'openai');
        const googleRequests = externalRequests.filter(req => req.metadata.provider === 'google');

        assertEquals(openaiRequests.length >= 1, true);
        assertEquals(googleRequests.length >= 1, true);

        console.log(`✅ Request logging verified! Total: ${requestLog.length}, External: ${externalRequests.length}`);
      ...
      const scenario: MockScenario = {
        ...
        generateResponse: (context, metadata) => {
          ...
          if (attemptCount < 3) {
            ...
            return RequestAnalyzer.generateErrorResponse(context, metadata, 'server_error');
          }
