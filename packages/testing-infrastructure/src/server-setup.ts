 ...
 */
export interface ServerTestEnvironment<TServer extends TestServer = TestServer, TClient extends TestClient = TestClient> {
  server: TServer;
  client: TClient;
 ...
 */
export type ServerStartupFunction<TServer extends TestServer = TestServer> = () => Promise<TServer>;

/**
 * Client factory function type
 */
export type ClientFactory<TClient extends TestClient = TestClient> = (baseUrl: string, timeout?: number) => TClient;

/**
 * Setup test environment with required environment variables
 */
export function setupServerTestEnvironment(config: ServerTestConfig = DEFAULT_SERVER_TEST_CONFIG): void {
 ...
 */
export function createServerTestConfig(overrides: Partial<ServerTestConfig> = {}): ServerTestConfig {
  return {
