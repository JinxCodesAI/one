...
async function runCommand(command: string, args: string[]): Promise<boolean> {
  try {
    ...
    console.log(
      `⚠️ Command failed: ${command} ${args.join(" ")} - ${error.message}`,
    );
    return false;
  }
}
...
async function main() {
  ...
  if (devCleaned) {
    ...
    console.log(
      "ℹ️ Development environment cleanup completed (may have been clean already)",
    );
  }
  ...
  if (prodCleaned) {
    ...
    console.log(
      "ℹ️ Production environment cleanup completed (may have been clean already)",
    );
  }
