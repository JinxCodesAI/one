...
describe("MessageInput Component Logic", () => {
  ...
  it("should handle disabled state logic", () => {
    ...
    const currentPlaceholder = isDisabled ? disabledPlaceholder : normalPlaceholder;
  ...
  it("should handle textarea auto-resize logic", () => {
    ...
    const multiLineHeight = baseHeight + (multiLine.split('\n').length - 1) * lineHeight;
  ...
  it("should handle keyboard event logic", () => {
    ...
    const shouldSubmitOnEnter = enterEvent.key === 'Enter' && !enterEvent.shiftKey;
    const shouldAddNewLineOnShiftEnter = shiftEnterEvent.key === 'Enter' && shiftEnterEvent.shiftKey;
