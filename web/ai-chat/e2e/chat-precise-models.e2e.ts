...
describe("E2E: Precise Model Testing", () => {
      ...
      generateResponse: (context, metadata) => {
        ...
        if (metadata.service === "openai" && model === "gpt-4.1-nano") {
          ...
          (body.choices as Array<{ message: { content: string } }>)[0].message
            .content = OPENAI_GPT4_NANO_RESPONSE;
        } else if (
          ...
          >)[0].content.parts[0].text = GOOGLE_GEMINI_FLASH_RESPONSE;
        } else if (
          ...
          (body.choices as Array<{ message: { content: string } }>)[0].message
            .content = ANTHROPIC_CLAUDE_RESPONSE;
        } else {
          console.log(
            `No custom response for service=${metadata.service}, model=${model}`,
          );
        }
  ...
  it("should test Anthropic Claude via OpenRouter with exact response", async () => {
    ...
    // Verify the request contained the correct model
    const lastOpenRouterRequest =
      openrouterRequests[openrouterRequests.length - 1];
    assertEquals(
      lastOpenRouterRequest.metadata.model,
  ...
  it("should verify mocking is working - no real API calls", async () => {
    ...
    externalRequests.forEach((req) => {
      ...
      assertEquals(
        ["openai", "google", "openrouter"].includes(req.metadata.service),
        true,
      );
    });
  });
  ...
  it("should handle conversation context with specific models", async () => {
    ...
    const requestBody = JSON.parse(
      JSON.stringify(lastRequest.context.body) || "{}",
    );
