/**
 * Main BFF Server Entry Point for AI Chat
 * 
 * This is the co-located BFF server implementation using the modular structure.
...
app.get('/health', async (c) => {
  try {
    ...
    const overallStatus = aiStatus.status === 'healthy' ? 'healthy' : 'degraded';
...
if (import.meta.main) {
  console.log(`🚀 AI Chat BFF Server starting...`);
  ...
  try {
    ...
    Deno.serve({
      ...
      onListen: ({ port, hostname }) => {
        ...
        console.log(`   - POST /api/ai/generate-object`);
        if (NODE_ENV === "production") {
