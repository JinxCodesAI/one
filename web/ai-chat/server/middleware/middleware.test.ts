...
Deno.test("JSON Validation Middleware - Invalid JSON", async () => {
  ...
  const req = new Request('http://localhost/test', {
    ...
    headers: {
      ...
      'Content-Length': '12' // Set content length for invalid json
    },
    body: 'invalid json'
  });
  ...
  // The middleware should catch the JSON parsing error
  if (res.status === 400) {
    const data = await res.json();
    assertEquals(data.error, 'Invalid JSON in request body');
  } else {
    // If Hono handles it differently, just verify it doesn't crash
    assertEquals(res.status < 500, true, "Should not return server error");
    await res.text(); // Consume response body
  }
});

Deno.test("JSON Validation Middleware - Empty Body", async () => {
  const app = new Hono();
  app.use('*', validateJSON);
