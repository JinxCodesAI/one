  ...
  await t.step("Vite proxy should route API calls to BFF", async () => {
    try {
      ...
      const endpoints = [
        ...
        { path: '/api/ai/models', expectedStatus: 200 },
        { path: '/api/profile/user-info', expectedStatus: 400 }, // 400 because no anonId
      ...
      for (const endpoint of endpoints) {
        ...
        assertEquals(response.status, endpoint.expectedStatus, 
          `${endpoint.path} should return ${endpoint.expectedStatus}`);
